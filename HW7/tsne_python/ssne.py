...
def ssne(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0):
    ...
    for iter in range(max_iter):
        ...
        num = -2. * np.dot(Y, Y.T)                                # -2 * Y * Y.T
        num = np.exp(-1. * np.add(np.add(num, sum_Y).T, sum_Y))   ##### CHANGE 
        ...
        PQ = P - Q
        for i in range(n):
            dY[i, :] = np.sum(np.tile(PQ[:, i], (no_dims, 1)).T * (Y[i, :] - Y), 0)     ##### CHANGE
...
if __name__ == "__main__":
    print("Run Y = tsne.tsne(X, no_dims, perplexity) to perform s-SNE on your dataset.")
